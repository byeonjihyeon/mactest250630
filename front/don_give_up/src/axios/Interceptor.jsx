	...
	instance.interceptors.response.use(
		...
		function(error) {   // 응답 처리 중 오류가 발생한 경우 (ResponseDTO.HttpStatus: 4xx, 5xx)
			...
			if(error.status == 403){ //토큰 유효시간 만료
				//status가 403인 경우는 accessToken이 만료되었을 때에도 응답되고 refreshToken이 만료되었을 대에도 응답됨

				if(error.config.headers.refreshToken === undefined //응답 코드가 403이면서 헤더에 refreshToken이 포함되지 않은 경우
					...
					Swal.fire({
						...
						if(result.isConfirmed){
							...
							//외부에서 브라우저 주소창을 자바스크립트로 바꿀 수 있게 해주는 도구인 histroy를 사용해야 함
			...
			}else{ //HttpStatus.INTERNAL_SERVER_ERROR == 500 (서버 오류 발생)
